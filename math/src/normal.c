...
{                               /* --- quantile of normal distrib. */
  ...
  if (prob >= 1.0) return  INFINITY;  /* check for limiting values */
  if (prob <= 0.0) return -INFINITY;  /* and return extrema */
...
double unitrand (double rand(void))
...
{                               /* --- cumulative distribution fn. */
  ...
  return unitcdf((mean-x) /sqrt(var));
}  /* normcdfQ() */
